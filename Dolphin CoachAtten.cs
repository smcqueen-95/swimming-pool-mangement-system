...
{
    public partial class Dolphin_CoachAtten : Form
    {
        public Dolphin_CoachAtten()
        {
            InitializeComponent();
        }
        ...
        {
            if (labelUser.Text == "Dolphin Team Leader")
            {
                this.Hide();
        ...
        private void Dolphin_CoachAtten_Load(object sender, EventArgs e)
        {
            labelUser.Text = GLOBAL.userType;

            //populating the datagridview with swimmer's data
            MySqlCommand command = new MySqlCommand("SELECT * FROM `coaches_atten` WHERE `Swim Team/s`='Dolphin'");
